...
BEGIN
  -- Completing a previously claimed single-file delete is safe and must stay
  -- recoverable after a whole-Project delete request begins.
  IF TG_OP = 'UPDATE'
    AND OLD.delete_attempt_id IS NOT NULL
    AND NEW.delete_attempt_id IS NULL
...
CREATE TRIGGER project_files_upload_deletion_fence
BEFORE INSERT OR UPDATE OF upload_attempt_id ON public.project_files
FOR EACH ROW EXECUTE FUNCTION public.project_file_deletion_fence();
CREATE TRIGGER project_files_delete_deletion_fence
BEFORE UPDATE OF delete_attempt_id ON public.project_files
FOR EACH ROW EXECUTE FUNCTION public.project_file_deletion_fence();
...
BEGIN
  ...
  FOR charged IN
    SELECT pf.storage_owner_id, pf.size_bytes
    FROM public.project_files AS pf
    WHERE pf.project_id = p_project_id
      AND pf.storage_charged
      AND pf.storage_owner_id IS NOT NULL
      AND pf.size_bytes > 0
    ORDER BY pf.id
    FOR UPDATE
  LOOP
    UPDATE public.user_storage
    SET bytes_used = greatest(0, bytes_used - charged.size_bytes),
        updated_at = now()
...
REVOKE ALL ON FUNCTION public.finalize_project_deletion(uuid, uuid, uuid)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.finalize_project_deletion(uuid, uuid, uuid)
  TO service_role;
