...
| Scheduled tasks, recurrence, pause, resume, retry and history                               | EXECUTION UNAVAILABLE          | Historical management and source contracts exist; creation/resume fail closed without the unified runner                |
| Current Work creation, queued prompts, approvals, background resume and rich deliverables   | BACKEND + RUNNER REQUIRED      | Historical /work viewer only; source and deployed runtime work remain                                                   |
| Durable autonomous background agents                                                        | BACKEND + PROVIDER REQUIRED    | One unified queue, isolated worker, leases, approval/recovery protocol, notifications and execution logs                |
