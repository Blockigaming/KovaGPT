...
The September 25 source adds `20260925000821`; the current **proposed** sequence
is 84 absent versions, three history-only records, 81 executable bodies and
182 final ledger rows. The older 80+3 hosted receipt does not validate this tree.
...
98-row remote ledger **84** source timestamps are absent. The default
push may skip older out-of-order versions; `--include-all` would include all
84, including a duplicate security body. The proposed sequence instead
requires **three history-only records first**, followed by only **81**
...
linked target and verify it selects exactly the remaining **81** source
