# Production operation approval packets (source draft, September 23, 2026)
...
`PROD_RG`, `PROD_APP`, subscription, previous revision, and previous immutable
digest are **unknown in this source document**; capture and review them from Azure
before filling any command below. A successful PLAN must verify the existing app's
full resource identity in that same subscription. The Azure web apps were stopped
in the last read-only inventory; do not assume a healthy previous revision exists.
