...
Captured 2026-09-23 21:31 UTC from Supabase project `mfbycmbjygcfkrsuepxf`,
...
(SHA-256 `c7be3f83413d9097cf5f80b700be0750bc93ff43bf3947654ddd4a0cb53fff84`)
...
They must run against the same target and checkpoint before interpreting their
results together. The first query ran in the bounded read-only transaction at
21:31 UTC; the second ran at 21:27 UTC. Both observed a 98-version ledger.

The comparison baseline is existing source migration
...
| Remote version   | Observed live scope                                                                                                                                                                                                                                                                                                         | Source-final distinction and remaining proof                                                                                                                                                                                                                                                                      |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `20260823215044` | Across 117 public tables, zero effective anon/authenticated TRUNCATE, REFERENCES, TRIGGER, or MAINTAIN privileges; zero effective sequence UPDATE grants.                                                                                                                                                                   | P also revokes from PUBLIC and covers sequence UPDATE and version-appropriate MAINTAIN. The aggregate current-state match cannot reconstruct which historical revoke occurred or certify later writer effects.                                                                                                    |
...
| `20260823215222` | Seven named trigger routines exist, postgres-owned, return `trigger`, have actual trigger bindings, and deny client EXECUTE. Six retain historical `search_path=public, pg_temp`; a seventh has another path.                                                                                                               | **Seven of seven** differ from the P candidate's `search_path=pg_catalog, public, pg_temp`. Compare full definitions and bindings to the isolated source capture; presence and EXECUTE denial alone are not complete proof.                                                                                       |
...
verifies that P clears those measured failures afterward. All four tests in
that file passed locally. The existing separate tests exercise caller-scope
...
current-state evidence with real differences from the proposed source-final
state. The remaining work is a complete same-checkpoint isolated source
