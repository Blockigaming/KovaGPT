...
- same-origin API routes for signup, password plus TOTP login, TOTP enrollment
  and removal, logout, session rotation, email verification, password recovery,
  and Google OAuth;
...
This is not the final cutover. Recovery-code sign-in and regeneration, passkey
adapters, the complete removal of `auth.users` compatibility principals,
production credential migration, and the production switch remain blocked on
their later release gates.
