...
A simultaneous anonymous Chromium capture was attempted against both production origins. The execution environment proxy rejected both tunnels (`ERR_TUNNEL_CONNECTION_FAILED`), so this change does **not** claim that the supplied visual baseline was freshly reproduced. The generated report preserves all six failures. Authenticated plan, populated-data, Safari/WebKit, screen-reader, and device-hardware observations remain blocked until lawful equivalent accounts and runners are supplied.

## Dispositions

- **Search semantics:** corrected by naming the `/library` destination “Library”; authenticated chat search remains an action rather than a misleading route.
