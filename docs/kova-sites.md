...
The database allocates Site and version IDs after checking mutation receipts. Callers use the returned IDs; retrying the same mutation returns its original allocation. A caller cannot choose a deleted Site's former hostname or a retired version's identity.
...
Cleanup removes each settled retirement record in the same transaction as its version and quota release. After all a deleted Site's obligations settle, it removes the Site row, title, URL aliases, viewers, and access sessions. No permanent owner or retired-ID tombstone remains. Opaque mutation receipts retain the existing eight-day retry window and then expire; replaying one cannot recreate deleted data. Account export first reserves every Site file's encoded JSON size against the remaining 50 MiB export limit using metadata-only pages, then verifies each immutable file body individually. Generic table readers share a cumulative allocation budget and use small pages for Work states and document bodies.
