...
While the Azure migration is incomplete, the repository enforces a migration cost freeze: the production Container App template accepts only `minReplicas = 0`, the production parameter example is zero, and the protected plan rejects any nonzero production replica setting. Re-enabling an always-on production replica requires a separate reviewed source change after migration activation is approved. This repository rule does not by itself change an already-running Azure resource or a separately created Container Apps job.
