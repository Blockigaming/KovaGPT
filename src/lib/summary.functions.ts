...
export type SummaryImage = { id: string; title: string; file_url: string | null; created_at: string };
export type SummaryFile = { id: string; title: string; file_url: string | null; file_type: string | null; created_at: string };
export type SummaryTask = { id: string; title: string; due_at: string | null; source: "project" | "scheduled"; project_id?: string };
export type GmailMessage = { id: string; from: string; subject: string; date: string; snippet: string };
export type CalendarEvent = { id: string; title: string; start: string | null; location: string | null; link: string | null };
...
export const getSummaryTasks = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }): Promise<SummaryTask[]> => {
    ...
    const schedTasks: SummaryTask[] = (sched.data ?? []).map((s: any) => ({
      id: s.id,
      title: s.title,
...
export const getGoogleStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }): Promise<GoogleStatus> => {
    ...
    const scopes = typeof data.scopes === "string" ? data.scopes.split(/\s+/).filter(Boolean) : (data.scopes as string[] | null) ?? [];
    return { connected: true, email: (data as any).email ?? null, scopes };
  });
  ...
  .handler(async ({ context }): Promise<{ available: boolean; messages: GmailMessage[] }> => {
    ...
    const details = await Promise.all(
      list.messages.slice(0, 5).map(async (m) => {
        ...
        const hv = (n: string) => j.payload?.headers?.find((h) => h.name.toLowerCase() === n.toLowerCase())?.value ?? "";
        return {
          id: j.id,
          from: hv("From"),
...
export const getCalendarSummary = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }): Promise<{ available: boolean; events: CalendarEvent[] }> => {
    ...
    const j = (await r.json()) as {
      items?: Array<{ id: string; summary?: string; start?: { dateTime?: string; date?: string }; location?: string; htmlLink?: string }>;
    };
