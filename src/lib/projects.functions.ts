...
export const listProjects = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<ProjectSummary[]> => {
    ...
    if (memErr) { console.error("[listProjects] mem", memErr.message); return []; }
...
export const getProject = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }): Promise<ProjectDetail | null> => {
    const { data: p, error } = await context.supabase
      ...
      .eq("id", data.id)
      .maybeSingle();
    if (error) { console.error("[getProject]", error.message); return null; }
    if (!p) return null;
    ...
    return {
      ...p,
      role: (mem?.role as ProjectRole | undefined) ?? (p.owner_id === context.userId ? "owner" : "viewer"),
    };
  });
...
export const updateProject = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ ok: true }> => {
    ...
    if (error) { console.error("[updateProject]", error.message); throw new Error("Failed to update project"); }
    return { ok: true };
  });
...
export const deleteProject = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ ok: true }> => {
    ...
    if (error) { console.error("[deleteProject]", error.message); throw new Error("Failed to delete project"); }
    return { ok: true };
  });
...
export const listMembers = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }): Promise<ProjectMember[]> => {
    ...
    for (const r of rows ?? []) {
      ...
      withEmail.push({ user_id: r.user_id, email, role: r.role as ProjectRole, created_at: r.created_at });
    }
    return withEmail;
  });
...
export const removeMember = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ ok: true }> => {
    ...
    if (error) { console.error("[removeMember]", error.message); throw new Error("Failed to remove member"); }
    return { ok: true };
  });
...
export const updateMemberRole = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ ok: true }> => {
    const { error } = await context.supabase
      ...
      .eq("project_id", data.project_id)
      .eq("user_id", data.user_id);
    if (error) { console.error("[updateMemberRole]", error.message); throw new Error("Failed to update role"); }
    return { ok: true };
  });
...
export const listInvites = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }): Promise<ProjectInvite[]> => {
    ...
    if (error) { console.error("[listInvites]", error.message); return []; }
...
export const inviteMember = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ id: string; auto_accepted: boolean }> => {
    ...
    } catch (e) { console.error("[inviteMember] lookup", e); }
    ...
    const { data: row, error } = await context.supabase
      ...
      .select("id")
      .single();
    if (error || !row) { console.error("[inviteMember]", error?.message); throw new Error("Failed to invite"); }

    if (existingUserId) {
      const { error: mErr } = await supabaseAdmin
        .from("project_members")
        .upsert({ project_id: data.project_id, user_id: existingUserId, role: data.role }, { onConflict: "project_id,user_id" });
...
export const revokeInvite = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ ok: true }> => {
    const { error } = await context.supabase
      ...
      .eq("id", data.id);
    if (error) { console.error("[revokeInvite]", error.message); throw new Error("Failed to revoke"); }
    return { ok: true };
  });
...
export const listMyPendingInvites = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<PendingInvite[]> => {
    ...
    const { data: rows, error } = await context.supabase
      ...
      .eq("status", "pending")
      .ilike("email", email);
    if (error) { console.error("[listMyPendingInvites]", error.message); return []; }
...
export const acceptInvite = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ project_id: string }> => {
    ...
    const { error: mErr } = await supabaseAdmin
      .from("project_members")
      .upsert({ project_id: invite.project_id, user_id: context.userId, role: invite.role }, { onConflict: "project_id,user_id" });
    if (mErr) { console.error("[acceptInvite] mem", mErr.message); throw new Error("Failed to accept"); }
...
export const declineInvite = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ ok: true }> => {
    ...
    const { error } = await supabaseAdmin
      ...
      .eq("id", data.invite_id)
      .ilike("email", email);
    if (error) { console.error("[declineInvite]", error.message); throw new Error("Failed"); }
    return { ok: true };
  });
...
export const listProjectChats = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }): Promise<ProjectChatSummary[]> => {
    const { data: rows, error } = await context.supabase
      ...
      .order("updated_at", { ascending: false })
      .limit(200);
    if (error) { console.error("[listProjectChats]", error.message); return []; }
...
export const createProjectChat = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ id: string }> => {
    const { data: row, error } = await context.supabase
      ...
      .select("id")
      .single();
    if (error || !row) { console.error("[createProjectChat]", error?.message); throw new Error("Failed to create chat"); }
    return { id: row.id };
  });
...
export const deleteProjectChat = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ ok: true }> => {
    ...
    if (error) { console.error("[deleteProjectChat]", error.message); throw new Error("Failed to delete"); }
    return { ok: true };
  });
