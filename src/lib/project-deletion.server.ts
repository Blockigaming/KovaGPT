...
async function verifyMetadataPaths(admin: ProjectDeletionAdmin, projectId: string): Promise<void> {
  let start = 0;
  while (true) {
...
export async function deleteProjectStorageFirst({
  ...
  try {
    ...
    const cleanup = await purgeProjectStorageFolder({
      ...
      onProgress: async () => {
        const { data: renewed, error: renewFailure } = await admin.rpc("renew_project_deletion", {
          p_attempt_id: attemptId,
          p_project_id: canonicalProjectId,
          p_user_id: userId,
        });
        if (renewFailure || renewed !== true) {
