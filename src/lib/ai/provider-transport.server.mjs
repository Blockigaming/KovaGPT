...
function abortReason(signal, phase) {
  ...
  return new ProviderTransportTimeoutError(phase);
}
...
export function createRequestDeadline(parentSignal, timeoutMs, phase = "provider_request") {
  ...
  return {
    ...
    normalize(error) {
      ...
      if (controller.signal.aborted && isAbortError(controller.signal.reason)) {
...
export function wrapResponseBodyWithDeadline(response, deadline, onFinish) {
  ...
  const onAbort = () => {
    ...
    if (!settle(deadline.didTimeout() ? "timeout" : "aborted", error)) return;
  ...
  const body = new ReadableStream({
    ...
    async pull(controller) {
      ...
      try {
        ...
        if (!settle(deadline.didTimeout() ? "timeout" : "failed", normalized)) return;
...
export async function fetchWithDeadline(fetchImpl, input, init, deadline, onFinish) {
  try {
    ...
    deadline.cleanup();
    onFinish?.({
      outcome: deadline.didTimeout() ? "timeout" : "failed",
      error: normalized,
    });
    throw normalized;
  }
