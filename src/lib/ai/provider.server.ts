...
export function providerUnavailableEnvelope(capability?: ProviderCapability): ProviderErrorEnvelope | null {
...
export function missingAiProviderResponse(fallback?: JsonObject): Response | null {
  ...
  return Response.json({ error: missing.error, code: missing.code, retryable: missing.retryable }, { status: missing.status });
}
...
async function providerFetch(path: string, capability: ProviderCapability, body: JsonObject, init?: RequestInit): Promise<Response> {
...
export async function streamingChatCompletions(body: JsonObject, init?: RequestInit): Promise<Response> {
