...
function normalizeResult(result: FirecrawlResult, id: number): WebSource | null {
  ...
  if (!url) return null;
  const title = cleanText(result.title || result.metadata?.title || domainFromUrl(url), 180) || domainFromUrl(url);
...
async function firecrawlSearch(apiKey: string, query: string, opts: { limit?: number; tbs?: string; signal?: AbortSignal } = {}): Promise<FirecrawlResult[]> {
...
export async function searchWeb(query: string, opts: { wantsNews?: boolean; limit?: number; signal?: AbortSignal } = {}): Promise<SearchResponse> {
  const apiKey = process.env.FIRECRAWL_API_KEY;
  const safeQuery = cleanText(query, 300);
  if (!safeQuery) return { status: "empty", query: "", sources: [], retryable: false, error: "Empty search query." };
  if (!apiKey) return { status: "not_configured", query: safeQuery, sources: [], retryable: false, error: "Search provider is not configured. Set FIRECRAWL_API_KEY on the server." };
  ...
  try {
    const [general, news] = await Promise.all([
      ...
      opts.wantsNews
        ? firecrawlSearch(apiKey, `${safeQuery} latest news`, { limit: 3, tbs: "qdr:w", signal: controller.signal })
...
export async function runWebSearch(query: string, wantsNews: boolean, signal?: AbortSignal): Promise<string | null> {
