import { chatCompletions, chatModel, providerErrorFromResponse, type JsonObject } from "@/lib/ai/provider.server";
import { searchWeb, type WebSource } from "@/lib/ai/search.server";

export type ResearchStageStatus = "pending" | "running" | "complete" | "failed";

export type ResearchStage = {
  id: "intake" | "plan" | "search" | "evidence" | "report";
  label: string;
...
type ResearchInsertResult = PromiseLike<{ error?: unknown }> & {
  select: (columns: string) => { maybeSingle: () => Promise<{ data?: { id?: string } | null; error?: unknown }> };
};

type ResearchPersistenceClient = {
  from: (table: string) => {
    insert: (values: JsonObject | JsonObject[]) => ResearchInsertResult;
    update: (values: JsonObject) => { eq: (column: string, value: string) => Promise<{ error?: unknown }> };
  };
};
...
async function createResearchRun(persistence: ResearchPersistence | undefined, query: string): Promise<string | null> {
...
async function updateResearchRun(persistence: ResearchPersistence | undefined, runId: string | null, values: JsonObject): Promise<void> {
...
function buildEvidence(plan: string[], sourceGroups: Array<{ query: string; sources: WebSource[] }>): { evidence: ResearchEvidence[]; sources: WebSource[] } {
  const evidence: ResearchEvidence[] = [];
  const sourcesByUrl = new Map<string, WebSource>();
  for (const group of sourceGroups) {
    for (const source of group.sources) {
      const key = source.url.toLowerCase().replace(/\/$/, "");
      if (!sourcesByUrl.has(key)) sourcesByUrl.set(key, { ...source, id: `src-${sourcesByUrl.size + 1}` });
...
async function writeReport(query: string, plan: string[], evidence: ResearchEvidence[], signal?: AbortSignal): Promise<string> {
  const upstream = await chatCompletions(
    {
...
export async function runDeepResearch(
  query: string,
  opts: { signal?: AbortSignal; onProgress?: (event: ResearchProgressEvent) => void | Promise<void>; persistence?: ResearchPersistence } = {},
): Promise<ResearchResult> {
  const safeQuery = sanitizeResearchText(query, 1000);
  if (!safeQuery) throw new Error("empty_research_query");
  const emit = async (stage: ResearchStage, progress: number) => {
    await opts.onProgress?.({ stage, progress });
  };

  const runId = await createResearchRun(opts.persistence, safeQuery);
  await emit({ id: "intake", label: "Scope research question", status: "complete", detail: safeQuery }, 0.1);
  await emit({ id: "plan", label: "Create research plan", status: "running" }, 0.2);
  let plan: string[];
  try {
  ...
  await emit({ id: "plan", label: "Create research plan", status: "complete", detail: `${plan.length} search queries` }, 0.32);

  await emit({ id: "search", label: "Search multiple source sets", status: "running" }, 0.38);
  const partialFailures: string[] = [];
  const sourceGroups = await Promise.all(
    plan.map(async (searchQuery) => {
      const response = await searchWeb(searchQuery, { wantsNews: true, limit: 4, signal: opts.signal });
      if (response.status !== "ok") partialFailures.push(`${searchQuery}: ${response.error ?? response.status}`);
      return { query: searchQuery, sources: response.sources };
    }),
  );
  await emit({ id: "search", label: "Search multiple source sets", status: "complete", detail: `${sourceGroups.reduce((n, g) => n + g.sources.length, 0)} raw sources` }, 0.58);

  await emit({ id: "evidence", label: "Extract and dedupe evidence", status: "running" }, 0.66);
  ...
  await emit({ id: "evidence", label: "Extract and dedupe evidence", status: "complete", detail: `${evidence.length} evidence notes` }, 0.76);

  await emit({ id: "report", label: "Write cited report", status: "running" }, 0.84);
  const report = await writeReport(safeQuery, plan, evidence, opts.signal);
  await updateResearchRun(opts.persistence, runId, { status: "complete", report, sources, partial_failures: partialFailures, completed_at: new Date().toISOString() });
  await emit({ id: "report", label: "Write cited report", status: "complete" }, 1);
