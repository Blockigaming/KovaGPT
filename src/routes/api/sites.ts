...
import { loadVerifiedWorkSiteOutput } from "@/lib/work-sites-output.mjs";
      ...
      POST: async ({ request }) => {
        ...
        try {
          ...
          if (action === "create" || action === "rename") {
            ...
            const v = payload(input.payload, ["versionId", "deliverableId", "workOutputId"]);
            if ((v.deliverableId == null) === (v.workOutputId == null)) throw new SiteInputError();
            if (v.workOutputId != null) {
              let inspected;
              try {
                inspected = await loadVerifiedWorkSiteOutput(
                  {
                    async readOutput(ownerId, outputId) {
                      const result = (await auth.supabaseUser
                        .from("work_execution_outputs" as never)
                        .select("id,owner_id,project_file_id,sha256,size_bytes,mime_type")
                        .eq("owner_id", ownerId)
                        .eq("id", outputId)
                    ...
                    async readProjectFile(_ownerId, fileId) {
                      const result = (await auth.supabaseUser
                        .from("project_files")
                    ...
                    async download(path) {
                      const result = await auth.supabaseUser.storage
                        .from("project-files")
                        .download(path);
                  ...
                  auth.userId,
                  siteUuid(v.workOutputId),
                );
              } catch {
                throw new SiteInputError("site_work_source_unverified");
              }
              values = { versionId: siteUuid(v.versionId), ...inspected };
              delete values.title;
            } else {
              const id = siteUuid(v.deliverableId);
              ...
              const downloaded = await auth.supabaseUser.storage
                .from(ref.bucket)
                .download(ref.path);
              if (
                downloaded.error ||
