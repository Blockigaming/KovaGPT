...
import { digest, parseArgs, summarizeRows, validateCases, writeJsonl } from "../../scripts/model-eval-contract.mjs";
...
import { buildBlindJudgeInput, parseJudgeResult, judgeCase, judgeRows } from "../../scripts/model-eval-judge-openai.mjs";
import { estimateCostUsd, executionOptions, extractOutputText, runOpenAiCase } from "../../scripts/model-eval-run-openai.mjs";

const cases = [
  { id: "a", category: "coding", weight: 1, prompt: "Return A", grader: { type: "exact", answer: "A" } },
  { id: "b", category: "safety_privacy", weight: 1, prompt: "Explain privacy", grader: { type: "rubric", criteria: ["Respect privacy"] } },
];
const rows = () => cases.map((item) => ({ id: item.id, category: item.category, weight: 1,
  status: "completed", score: 1, output: "A", cost_usd: 0.02, latency_ms: 100 }));
const mockOptions = { apiKey: "test-only-not-a-real-key", model: "gpt-5.6-sol", item: cases[0],
  reasoningEffort: "high", maxOutputTokens: 4096, timeoutMs: 50, inputUsdPerMtok: 4, outputUsdPerMtok: 20 };
...
for (const value of [null, undefined, "", "1", false, true, [], {}, Number.NaN, Infinity, -0.1, 1.1]) {
  test(`strict score rejects ${String(value)} (${typeof value})`, () => {
    const bad = rows(); bad[0].score = value;
    assert.throws(() => scoreRun(cases, bad), /score/u);
    assert.throws(() => compareRuns(rows(), bad), /score/u);
    assert.throws(() => parseJudgeResult(JSON.stringify({ score: value, rationale: "test" })), /score/u);
  });
}
...
test("missing or partial costs stay null with coverage and suppress ratios", () => {
  for (const costs of [[null, null], [0.01, undefined]]) {
...
test("partial latency does not become a falsely fast full-run mean", () => {
  const input = rows(); input[0].latency_ms = null;
...
test("all unsuccessful response statuses score zero even with a convincing output", () => {
  for (const status of ["error", "timeout", "failed", "incomplete", "cancelled"]) {
    const raw = rows(); raw[0].status = status;
...
test("semantic answers and number mentions need review, not substring credit", () => {
  ...
  assert.equal(gradeDeterministic({ grader: { type: "exact_semantic", answer: "Canberra" } }, "Canberra"), null);
});
...
test("matching IDs cannot hide a category, weight, or content change", () => {
  for (const change of [{ category: "factuality" }, { weight: 2 }]) {
    const input = rows(); input[0] = { ...input[0], ...change };
...
test("weighted scoring and floating-point deltas use numerical tolerances", () => {
  const reference = rows(); reference[1].latency_ms = 200;
  const candidate = rows(); candidate[0].score = 0.9;
  candidate[0].latency_ms = 80; candidate[1].latency_ms = 120;
  candidate.forEach((row) => { row.cost_usd = 0.01; });
  ...
  assert.equal(summarizeRows([{ ...reference[0], weight: 3, score: 0 }, reference[1]]).overall_score, 0.25);
});

test("blind judge includes reference answer without provider or model metadata", () => {
  const input = JSON.parse(buildBlindJudgeInput({ ...cases[0], provider: "secret-provider", model: "secret-model" }, "answer"));
  assert.equal(input.reference_answer, "A");
  assert.equal("model" in input, false); assert.equal("provider" in input, false);
...
test("mock transport is non-stored, disallows redirects and records output/usage", async () => {
  let request;
  const row = await runOpenAiCase({ ...mockOptions, fetcher: async (url, init) => {
    request = { url, ...init, body: JSON.parse(init.body) };
    return response({ id: "test", status: "completed", output_text: "A", usage: { input_tokens: 100, output_tokens: 25 } });
  } });
  assert.equal(request.url, "https://api.openai.com/v1/responses");
  assert.equal(request.body.store, false); assert.equal(request.redirect, "error");
  assert.equal(row.status, "completed"); assert.equal(row.cost_usd, 0.0009);
});
...
test("incomplete responses stay failed and missing provider status is not completed", async () => {
  for (const status of ["incomplete", "failed", undefined]) {
    const row = await runOpenAiCase({ ...mockOptions, fetcher: async () => response({ status, output_text: "A" }) });
    assert.notEqual(row.status, "completed"); assert.equal(row.score, 0);
  }
});
...
test("deadline also covers a fetch or body parser that ignores AbortSignal", async () => {
  for (const fetcher of [async () => new Promise(() => {}), async () => ({ ok: true, json: async () => new Promise(() => {}) })]) {
    ...
    assert.equal(row.status, "timeout"); assert.equal(row.cost_usd, null);
  }
});

test("HTTP and exception diagnostics do not echo secrets or private provider bodies", async () => {
  for (const fetcher of [async () => new Response("private-secret", { status: 401 }), async () => { throw new Error("private-secret"); }]) {
    const row = await runOpenAiCase({ ...mockOptions, fetcher });
    assert.equal(row.status, "error"); assert.ok(!JSON.stringify(row).includes("private-secret"));
  }
});

test("legitimate refusals remain gradable and all output text chunks are collected", () => {
  assert.equal(extractOutputText({ output: [{ type: "message", content: [{ type: "refusal", refusal: "Cannot disclose private data." }] }] }), "Cannot disclose private data.");
...
test("judge failures cannot manufacture fully graded results", async () => {
  await assert.rejects(judgeCase({ ...mockOptions, output: "answer", timeoutMs: 5, fetcher: async () => new Promise(() => {}) }), /unsuccessful: timeout/u);
  await assert.rejects(judgeCase({ ...mockOptions, output: "answer", fetcher: async () => response({ status: "completed", output_text: '{"score":null,"rationale":"bad"}' }) }), /score/u);
});

test("complete mock pipeline grades rubric and retains category and scoring provenance", async () => {
  const graded = await judgeRows(cases, rows(), { ...mockOptions,
    fetcher: async () => response({ status: "completed", output_text: '{"score":0.75,"rationale":"Partial credit"}', usage: { input_tokens: 100, output_tokens: 25 } }) });
  assert.equal(graded[0].score, 1); assert.equal(graded[1].score, 0.75);
...
test("CLI execution is opt-in and bounded; malformed options cannot enable spending", () => {
  ...
  assert.throws(() => executionOptions(new Map([["execute", true], ["max-requests", "29"]]), 30), /max-requests/u);
  assert.equal(executionOptions(new Map([["execute", true], ["max-requests", "30"]]), 30).maxOutputTokens, 4096);
});
