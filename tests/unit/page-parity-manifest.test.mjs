...
test("route manifest includes reusable public, publishing, developer, assistant and locale templates", () => {
  ...
  assert.equal(routes.reviewedPublicRouteCount, 517);
});
...
test("all 517 reconciled public routes are reviewed and the sitemap retains 167 substantive routes", async () => {
  ...
  assert.equal(review.reviewedRouteCount, 517);
  assert.equal(new Set(review.records.map((entry) => entry.route)).size, 517);
  ...
  assert.equal(PUBLIC_REVIEW_PATHS.length, 517);
...
test("release route manifest is generated from all route files and one sitemap source", async () => {
  ...
  assert.equal(manifest.reviewedPublicRouteCount, 517);
