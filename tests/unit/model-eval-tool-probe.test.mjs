...
test("correct tool calls with a wrong final result fail", async () => {
  const result = await runToolProbe(
    scripted(response(limit(), count()), response(final('{"remaining":3}'))),
  );
  assert.equal(result.score, 0);
...
test("repeated tool names and reused call IDs fail without repeated execution", async () => {
  const repeated = await runToolProbe(
    scripted(response(limit()), response({ ...limit(), call_id: "new-id" })),
  );
  assert.equal(repeated.reason, "repeated_tool");
  const duplicate = await runToolProbe(
    scripted(response(limit()), response({ ...count(), call_id: "get_project_limit" })),
  );
...
test("timeouts, provider failures and model changes cannot earn passing scores", async () => {
  ...
  const failed = await runToolProbe(async () => {
  ...
  const changed = await runToolProbe(
    scripted(response(limit()), { ...response(count()), model: "other" }),
  );
