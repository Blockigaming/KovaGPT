...
test('extractOutputText supports Responses API output_text and message content', () => {
  ...
  assert.equal(extractOutputText({ output: [{ type: 'message', content: [{ type: 'output_text', text: 'hello' }, { type: 'output_text', text: ' world' }] }] }), 'hello world');
});
...
test('runOpenAiCase sends an isolated non-stored Responses request and records usage', async () => {
  ...
  const fetcher = async (url, init) => {
    ...
    return new Response(JSON.stringify({ id: 'resp_test', status: 'completed', output_text: 'answer', usage: { input_tokens: 100, output_tokens: 25 } }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  };
  const row = await runOpenAiCase({ apiKey: 'test-key', item: { id: 'case-1', prompt: 'test prompt' }, model: 'gpt-5.6-sol', reasoningEffort: 'high', maxOutputTokens: 4096, timeoutMs: 1000, inputUsdPerMtok: 4, outputUsdPerMtok: 20, fetcher });
