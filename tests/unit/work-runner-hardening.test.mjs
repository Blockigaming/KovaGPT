  ...
  const result = await provider.reason(
    {
      ...
      maxOutputTokens: 100,
      phase: "coordinator",
      coordinatorObjective: "Analyze CSV",
      objective: "Analyze CSV",
      sessionContext: null,
      specialist: null,
      specialistResults: [],
      directions: [],
...
test("provider Site output is one verified importable artifact and never a publication directive", async () => {
  const provider = configuredProvider(
    {
      responsesUrl: "https://provider.example.net/responses",
      providerKey: "k".repeat(32),
    ...
    async (_url, request) => {
      assert.match(
        JSON.parse(request.body).input[0].content,
        /only an importable unpublished artifact/u,
      );
      return Response.json({
        output_text: JSON.stringify({
          ...
          title: "Status board",
          files: [
            { path: "index.html", content: "<!doctype html><title>Status board</title>" },
  ...
  const result = await provider.reason(
    {
      ...
      maxOutputTokens: 100,
      phase: "coordinator",
      coordinatorObjective: "Create a Site",
  ...
  assert.equal(result.receipt.directive, undefined);
  assert.equal(result.receipt.outputs.length, 1);
  assert.equal(result.receipt.outputs[0].mimeType, "application/json");
  assert.ok(result.receipt.outputs[0].bytes > 0);
  const stored = JSON.parse(Buffer.from(result.artifacts[0].contentBase64, "base64").toString());
  assert.equal(stored.schema, "kova-work-site-v1");
  assert.equal(stored.title, "Status board");
  assert.deepEqual(
