...
test("operator wrapper remains read-only and contains exactly one container exec", async () => {
  ...
  assert.match(wrapper, /ssl:\s*\{/u);
  assert.match(wrapper, /rejectUnauthorized:\s*false/u);
  assert.doesNotMatch(wrapper, /pg_catalog\\.pg_stat_ssl/u);
  assert.doesNotMatch(wrapper, /ssl !== "true"/u);
  assert.match(wrapper, /for command in az jq node sha256sum openssl mktemp script/u);
  assert.doesNotMatch(wrapper, /for command in az jq node psql/u);
  assert.doesNotMatch(wrapper, /"\\\\conninfo"/u);
  assert.doesNotMatch(wrapper, /destination count query lacked client TLS evidence/u);
  assert.match(wrapper, /require\("pg"\)/u);
  assert.match(wrapper, /ssl:\s*\{/u);
  assert.match(wrapper, /rejectUnauthorized:\s*false/u);
  assert.match(wrapper, /application_name:\s*"auth-rehearsal-strict-db-probe-precheck"/u);
  ...
  assert.doesNotMatch(wrapper, /for command in az jq node psql/u);
  assert.match(wrapper, /require\("pg"\)/u);
  assert.match(wrapper, /SELECT[\s\S]*count\(\*\) FROM auth\.users/u);
  assert.match(wrapper, /count\(\*\) FROM auth\.identities/u);
  assert.ok(wrapper.includes(`--command "sh -c 'stty -echo; exec sh'"`));
  assert.ok(!wrapper.includes("PROBE_STDIN_COMMAND=$'stty -echo"));
