...
test('blind judge payload omits candidate identity', () => {
  const text = buildBlindJudgeInput({ prompt: 'Explain X', grader: { criteria: ['correct', 'clear'] } }, 'Answer A');
...
test('judge parser accepts bounded score and rejects invalid score', () => {
  assert.deepEqual(parseJudgeResult('{"score":0.75,"rationale":"Mostly correct"}'), { score: 0.75, rationale: 'Mostly correct' });
