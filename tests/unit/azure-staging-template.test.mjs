...
test("validator rejects Lovable, mutable image, production target, and enabled generation drift", async () => {
  ...
  assert.throws(() =>
    validateAzureStagingTemplate({ template: `${template}\n// LOVABLE_API_KEY`, parameters }),
  );
  assert.throws(() =>
...
test("template uses managed identity for ACR, Key Vault, and Azure OpenAI", async () => {
  const template = await readFile("infra/azure/staging/main.bicep", "utf8");
  assert.match(template, /type: 'UserAssigned'/u);
  assert.match(template, /keyVaultUrl: supabaseServiceRoleSecretUri/u);
  assert.match(template, /scope: azureOpenAi/u);
  assert.match(template, /roleDefinitionId: cognitiveServicesOpenAiUserRoleDefinitionId/u);
  assert.match(template, /name: 'AZURE_CLIENT_ID'/u);
  assert.match(template, /value: identity\.properties\.clientId/u);
  assert.doesNotMatch(template, /openAiSecretUri|openai-api-key/iu);
  assert.doesNotMatch(template, /name: '(?:OPENAI_API_KEY|AZURE_OPENAI_API_KEY)'/u);
});
