...
-- Browser Range/string offsets are UTF-16 code-unit positions. PostgreSQL
-- char_length counts Unicode code points, so supplementary characters (emoji)
-- require two units to match JavaScript selectionStart/selectionEnd.
...
revoke all on function public.utf16_code_unit_length(text) from public, anon;
grant execute on function public.utf16_code_unit_length(text) to authenticated, service_role;

alter table public.chat_message_versions
  drop constraint if exists chat_message_versions_selection_range;
...
comment on function public.utf16_code_unit_length(text)
  is 'Returns JavaScript-compatible UTF-16 code-unit length for selected-text offsets.';
;
