...
--
-- Goals:
-- 1. Due scheduled tasks are claimed atomically.
-- 2. Only trusted service-role execution may claim work.
-- 3. Concurrent workers cannot execute the same occurrence.
-- 4. Expired leases can be recovered safely.
-- 5. User ownership remains enforced by the existing scheduled_tasks RLS policy.
--
-- This migration creates execution primitives only. It does not claim that a
...
revoke all on function public.claim_due_scheduled_tasks(text, integer, integer)
  from public, anon, authenticated;

grant execute on function public.claim_due_scheduled_tasks(text, integer, integer)
  to service_role;
...
revoke all on function public.recover_expired_scheduled_task_leases()
  from public, anon, authenticated;

grant execute on function public.recover_expired_scheduled_task_leases()
  to service_role;
...
begin
  ...
  set
    ...
    last_error = left(
      coalesce(nullif(trim(p_safe_error), ''), 'Scheduled task failed.'),
      500
    ),
    updated_at = now()
  where id = p_task_id;
...
revoke all on function public.complete_scheduled_task_execution(
  uuid,
  text,
  timestamptz,
  text
...
revoke all on function public.fail_scheduled_task_execution(
  uuid,
  text,
  text,
  text,
...
grant execute on function public.complete_scheduled_task_execution(
  uuid,
  text,
  timestamptz,
  text
...
grant execute on function public.fail_scheduled_task_execution(
  uuid,
  text,
  text,
  text,
