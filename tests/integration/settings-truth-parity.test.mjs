...
test("data controls describe only the removed local-only switches", () => {
  ...
  assert.match(
    settings,
    /removed model-improvement switch changed only a\s+browser-local value/,
  );
  assert.match(
    settings,
    /was not wired to an account-level or\s+AI-provider training control/,
  );
  assert.match(
    settings,
  ...
  assert.doesNotMatch(
    settings,
    /Device settings are not sent to an AI provider/,
  );
