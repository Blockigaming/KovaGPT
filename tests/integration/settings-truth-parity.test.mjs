...
test("paid billing remains reachable and every unavailable state has a truthful next action", () => {
  ...
  assert.doesNotMatch(settings, /hideSubscription|tabs\.filter\(\(t\) => t\.v !== "subscription"\)/);
...
test("data controls do not present device values as provider controls", () => {
  ...
  assert.match(settings, /does not expose an account-level model-training switch/);
  assert.match(settings, /Device settings\s+are not sent to an AI provider/);
  assert.match(settings, /does not present browser\s+switches as account-level or AI-provider controls/);
