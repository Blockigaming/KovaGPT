...
test("Project deletion is durable, owner-only, lease-fenced, and Storage-first", () => {
  ...
  assert.match(migration, /pf\.status = 'pending'[\s\S]*pf\.upload_lease_until/);
  assert.match(migration, /pf\.status = 'deleting'[\s\S]*pf\.delete_lease_until/);
  assert.match(migration, /project_files_upload_deletion_fence/);
  assert.match(migration, /project_files_delete_deletion_fence/);
  assert.match(
    migration,
