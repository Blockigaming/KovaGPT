...
test("public capability copy excludes retired or unsupported claims", () => {
  ...
  assert.match(llms, /Free uses Instant/);
  assert.match(llms, /Thinking control is an Upgrade to Plus action, not an executable Free mode/);
  assert.match(llms, /Plus Chat includes Instant, Medium, and Thinking/);
  assert.match(llms, /the existing High\/Nova compute route/);
  assert.match(llms, /Pro Chat includes Instant, Medium, High, Extra High, Max, and Ultra/);
  assert.doesNotMatch(llms, /Free includes Instant and Thinking modes/);
