  ...
  for (const [path, sourcePath] of ROUTE_SOURCE) {
    ...
    assert.ok(
      source.includes("https://kovagpt.com" + path) ||
        (source.includes("seoLandingHead") &&
    ...
    const publicAssets = [
      ...source.matchAll(
        /(?:https:\/\/kovagpt\.com)?(\/(?:og\/[^"'\s)]+|favicon[^"'\s)]*))/gu,
      ),
    ].map((match) => match[1]);
...
test("public research copy states provider and verification limits without guarantees", async () => {
  ...
  assert.match(
    sources[0],
    /not affiliated with,\s*endorsed by, or sponsored by/iu,
  );
  assert.doesNotMatch(
    sources[0],
    /best general-purpose default|best for long documents/iu,
  );
});
...
test("privacy avoids unsupported security, retention, export, and training guarantees", async () => {
  ...
  assert.match(
    privacyText,
    /some conversation history is kept on your device/iu,
  );
  assert.match(
    privacyText,
    /Third-party providers process data under the provider terms/iu,
  );
  assert.match(
    privacyText,
    /does not include a Kova-owned model-training workflow/iu,
  );
  assert.match(
    privacyText,
    /Authorized operators may access account or content data/iu,
  );
...
test("capability landing pages do not promise detector evasion, legal rights, or verified output", async () => {
  ...
  assert.match(
    combined,
    /does not guarantee that an output is clear of third-party rights/iu,
  );
  assert.match(
    combined,
    /does not promise that rewritten text will receive a particular score/iu,
  );
  assert.match(combined, /not guaranteed to compile/iu);
  assert.match(combined, /verify the result/iu);
  assert.match(
    combined,
    /does not expose output-format or transparency controls/iu,
  );
...
test("public copy does not advertise retired modes, voice, or unlimited/free provider features", async () => {
  const combined = (
    await Promise.all([...ROUTE_SOURCE.values()].map(readSource))
  ).join("\n");
...
test("the shared public footer carries product and affiliation caveats", async () => {
  ...
  assert.match(
    footerText,
    /features can depend on plan eligibility and external providers/iu,
  );
  assert.match(footerText, /independently developed/u);
  assert.match(
    footerText,
    /do not imply sponsorship, endorsement, or affiliation/u,
  );
