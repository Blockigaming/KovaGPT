...
test("Temporary Chat enforces clean or personalized context without new memory", () => {
  ...
  assert.match(page, /isStreaming=\{isStreaming\}/);
  assert.match(page, /disabled=\{!principalReady\}/);
  ...
  assert.match(chatStore, /branchRootId: active\.id/);
  assert.match(chatStore, /branchOrigin: undefined/);
  assert.match(
    chatStore,
