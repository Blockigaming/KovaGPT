...
const JUDGE_INSTRUCTIONS =
  "Evaluate the answer against the task, rubric, and reference answer. " +
...
export async function judgeCase({
  ...
  const result = await runOpenAiCase({
    apiKey,
    model,
    item: {
      id: item.id,
      input: [
        ...
        {
          role: "user",
          content: [{ type: "input_text", text: buildBlindJudgeInput(item, output) }],
        },
      ],
    },
  ...
  if (result.status !== "completed")
    throw new Error(`Judge request unsuccessful: ${result.status}`);
  return {
    ...parseJudgeResult(result.output),
...
async function main() {
  ...
  const input = await readJsonl(
    args.get("grades") ?? "artifacts/model-eval/deterministic-grades.jsonl",
  );
  const prepared = gradeRows(cases, input);
  if (indexRows(input).size !== cases.length)
    throw new Error("Judge input must cover the whole suite");
      ...
      JSON.stringify(
        {
          ...
          note: "Dry run: no API calls or spending.",
        },
        null,
        2,
      ),
  ...
  if (
    ...
    throw new Error(
      "An API credential and explicit judge input/output price assumptions are required",
    );
  }
  ...
  try {
    const rows = await judgeRows(cases, input, {
      apiKey: process.env.OPENAI_API_KEY,
      model: args.get("model") ?? process.env.KOVA_EVAL_JUDGE_MODEL ?? "gpt-5.6-sol",
      timeoutMs,
      inputUsdPerMtok,
      outputUsdPerMtok,
