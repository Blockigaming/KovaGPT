...
LOG_DIR="$(mktemp -d "${TMPDIR:-/tmp}/kova-scheduled-time-worker.XXXXXX")"
printf 'SOURCE_SHA=%s\nLOG_DIRECTORY=%s\n' "$SOURCE_SHA" "$LOG_DIR"
...
gate node-syntax node --check tests/unit/scheduled-time-semantics-v2.test.mjs
gate worker-test-syntax node --check tests/unit/scheduled-worker-v2.test.mjs
gate worker-build-test-syntax node --check tests/unit/scheduled-worker-build-v2.test.mjs
...
gate scheduler-source-tests node --test \
  tests/unit/scheduled-time-semantics-v2.test.mjs \
  tests/unit/scheduled-worker-v2.test.mjs \
  tests/unit/scheduled-worker-build-v2.test.mjs \
...
# A built worker without the explicit enable flag must stop before constructing
# the Supabase client or attempting any provider/database operation.
set +e
...
printf '\n============================================================\n'
printf ' KOVAGPT_SCHEDULED_TIME_AND_WORKER_V2_SOURCE=PASS\n'
printf ' SOURCE_SHA=%s\n' "$SOURCE_SHA"
printf ' MIGRATION_COUNT=85\n'
