...
export function verifyAiProviderContract({ provider, transport = "", catalog, staging }) {
  ...
  const requiredProviderPatterns = [
    ...
    /IDENTITY_ENDPOINT/u,
    /IDENTITY_HEADER/u,
    /AZURE_FOUNDRY_MANAGED_IDENTITY_RESOURCE = "https:\/\/ai\.azure\.com"/u,
    /AZURE_COGNITIVE_SERVICES_MANAGED_IDENTITY_RESOURCE =\s*"https:\/\/cognitiveservices\.azure\.com"/u,
    /managedIdentityResourceForAzureBaseUrl\(target\.baseUrl\)/u,
    /searchParams\.set\("resource", resource\)/u,
