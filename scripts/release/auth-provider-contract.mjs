...
export function inspectAuthProviderContract({ files = trackedFiles() } = {}) {
  const errors = [];
  ...
  if (existsSync(lockPath)) {
    ...
    for (const name of Object.keys(lock.packages?.[""]?.dependencies ?? {})) {
      if (name.startsWith("@clerk/")) errors.push(`package-lock.json:root:${name}`);
    }
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const errors = inspectAuthProviderContract();
  if (errors.length) {
    console.error(`Supabase auth ownership contract failed:\n${errors.join("\n")}`);
    process.exit(1);
  }
  console.log("AUTH_PROVIDER_CONTRACT=PASS provider=supabase clerkRuntime=false");
}
