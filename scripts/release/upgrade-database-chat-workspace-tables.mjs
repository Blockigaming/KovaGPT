...
export const CHAT_WORKSPACE_TABLE_FILE = "upgrade-chat-workspace-table-catalog.json";
export const CHAT_WORKSPACE_TABLE_NAMES = Object.freeze([
  "chat_branches",
  "chat_custom_rules",
...
// Read PostgreSQL catalogs, not chat contents; no application routine is invoked.
// Unsupported replication, maintenance, role-authority and storage layouts are
// excluded here and rejected by the exact-four-table validator, never treated as
...
export const CHAT_WORKSPACE_TABLE_SQL = `begin transaction isolation level repeatable read read only;
set local statement_timeout = '10s';
set local lock_timeout = '1s';
set local search_path = pg_catalog;
with target as (
 select c.* from pg_class c join pg_namespace n on n.oid=c.relnamespace
 where n.nspname='public' and c.relname in ('chat_branches','chat_custom_rules','chat_message_versions','chat_pinned_files')
...
select jsonb_build_object(
 'schemaVersion',1,'captureKind','chat-workspace-table-catalog',
...
commit;`;
export const CHAT_WORKSPACE_TABLE_QUERY_SHA256 = digest(CHAT_WORKSPACE_TABLE_SQL);
const fail = () => {
  throw new Error("upgrade_chat_workspace_tables_invalid");
};
...
const table = object({
  ...
  name: literal(...CHAT_WORKSPACE_TABLE_NAMES),
  kind: literal("r"),
  owner: text,
...
const captureRule = object({
  ...
  captureKind: literal("chat-workspace-table-catalog"),
  ...
  tableCount: literal(4),
  tables: array(table, named, 4, 4),
});

export function validateChatWorkspaceTableCapture(input, expectedVersions) {
  ...
  if (
    ...
    !["20260823151901", "20260824085444"].every((v) => expected.includes(v))
  )
    fail();
  if (
    JSON.stringify(capture.tables.map((r) => r.name)) !== JSON.stringify(CHAT_WORKSPACE_TABLE_NAMES)
  )
    fail();
...
export function parseChatWorkspaceTableCapture(stdout, expectedVersions) {
  if (typeof stdout !== "string" || Buffer.byteLength(stdout) > 2 * 1024 * 1024) fail();
  let capture;
  try {
  ...
  return validateChatWorkspaceTableCapture(capture, expectedVersions);
}
...
export function buildChatWorkspaceTableEvidence({
  baseline,
  upgraded,
  baselineVersions,
  finalVersions,
  ...
  const before = validateChatWorkspaceTableCapture(baseline, baselineVersions),
    after = validateChatWorkspaceTableCapture(upgraded, finalVersions);
  ...
  return {
    schemaVersion: 1,
    captureKind: "chat-workspace-table-checkpoints",
    sourceCommit,
    sourceTree,
    ...
    limitations: [
      "Only the four named ordinary public tables; inheritance, rules, effective publications, relation/TOAST/column storage options, nondefault column statistics targets or planner options, extended statistics, non-heap access methods, typed tables, non-origin capture sessions or non-origin internal constraint triggers, noncanonical API-role authority, nondefault tablespaces and a named PUBLIC role fail closed. Declared array dimensions, dropped attribute slots and inbound foreign keys are captured explicitly.",
