...
export function analyzeMigrationManifest(manifest) {
  ...
  for (const [index, migration] of migrations.entries()) {
    if (migration.order !== index + 1) throw new Error(`migration_order_invalid:${migration.filename}`);
    ...
    if (!SHA256.test(migration.sha256 ?? "")) throw new Error(`migration_sha_invalid:${migration.filename}`);
  ...
  if (manifest.latest !== migrations.at(-1).filename) throw new Error("migration_manifest_latest_mismatch");
...
export function normalizeRemoteVersions(value) {
  ...
  return [...new Set(candidates.map((item) => {
    const raw = typeof item === "string" ? item : String(item?.version ?? item?.timestamp ?? item?.name ?? "");
    const match = raw.match(/\d{14}/u);
    return match?.[0] ?? "";
  }).filter((version) => VERSION.test(version)))].sort();
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (ready) {
    ...
    if (targetRef === productionRef && process.env.KOVA_PRODUCTION_MIGRATION_APPROVED !== targetRef) {
