#!/usr/bin/env bash
set -Eeuo pipefail

cd "$(dirname "${BASH_SOURCE[0]}")/../.."

EXPECTED_BRANCH="finalization/checkpoint-2026-08-28"
if [ "$(git branch --show-current)" != "$EXPECTED_BRANCH" ]; then
  echo "STOP=WRONG_BRANCH"
  exit 1
fi
if ! git diff --quiet || ! git diff --cached --quiet; then
  echo "STOP=TRACKED_OR_STAGED_CHANGES"
  git status --short --branch
  exit 1
fi

SOURCE_SHA="$(git rev-parse HEAD)"
LOG_DIR="$(mktemp -d "${TMPDIR:-/tmp}/kova-scheduled-v2.XXXXXX")"
printf 'SOURCE_SHA=%s\nLOG_DIRECTORY=%s\n' "$SOURCE_SHA" "$LOG_DIR"

gate() {
  local name="$1"
  shift
  printf '\n=== %s ===\n' "$name"
  if "$@" >"$LOG_DIR/$name.log" 2>&1; then
    printf '%s=PASS\n' "$name"
    tail -n 16 "$LOG_DIR/$name.log"
  else
    local code=$?
    printf '%s=FAIL\n' "$name"
    tail -n 220 "$LOG_DIR/$name.log"
    printf 'LOG_DIRECTORY=%s\n' "$LOG_DIR"
    exit "$code"
  fi
}

for tool in eslint prettier tsc; do
  if [ ! -x "node_modules/.bin/$tool" ]; then
    printf 'STOP=MISSING_LOCAL_TOOL_%s\n' "$tool"
    exit 1
  fi
done

gate node-syntax node --check tests/unit/scheduled-execution-v2-schema.test.mjs
gate engine-test-syntax node --check tests/unit/scheduled-execution-v2-engine.test.mjs

gate targeted-format node_modules/.bin/prettier --check \
  src/lib/scheduled-execution-v2.server.ts \
  src/lib/scheduled-tasks.functions.ts \
  tests/unit/scheduled-execution-v2-schema.test.mjs \
  tests/unit/scheduled-execution-v2-engine.test.mjs

gate targeted-lint node_modules/.bin/eslint \
  src/lib/scheduled-execution-v2.server.ts \
  src/lib/scheduled-tasks.functions.ts \
  tests/unit/scheduled-execution-v2-schema.test.mjs \
  tests/unit/scheduled-execution-v2-engine.test.mjs

gate migration-contract node scripts/release/migrations.mjs

gate v2-unit node --test \
  tests/unit/scheduled-execution-v2-schema.test.mjs \
  tests/unit/scheduled-execution-v2-engine.test.mjs

gate legacy-scheduler-regression node --test tests/unit/scheduled-batch-safety.test.mjs

gate typecheck node_modules/.bin/tsc --noEmit

grep -nF 'export const scheduledExecutionAvailable = false;' src/lib/scheduled-tasks.functions.ts \
  >"$LOG_DIR/fail-closed.log"

git diff --check
if ! git diff --quiet || ! git diff --cached --quiet; then
  echo "STOP=VALIDATION_CHANGED_TRACKED_FILES"
  git status --short --branch
  exit 1
fi

printf '\n============================================================\n'
printf ' KOVAGPT_SCHEDULED_EXECUTION_V2_SOURCE=PASS\n'
printf ' SOURCE_SHA=%s\n' "$SOURCE_SHA"
printf ' SCHEMA_APPLIED_TO_PRODUCTION=NO\n'
printf ' SCHEDULER_ENABLED=NO\n'
printf ' PROVIDER_LIVE_CALLS=0\n'
printf ' BUILD_RUN=NO\n'
printf ' BROWSER_MATRIX_RUN=NO\n'
printf ' LEDGER_PROMOTED=NO\n'
printf ' GITHUB_ACTIONS_DISPATCHED=0\n'
printf ' AZURE_DEPLOYMENTS=0\n'
printf ' LOVABLE_CREDITS_USED=0\n'
printf '============================================================\n'
